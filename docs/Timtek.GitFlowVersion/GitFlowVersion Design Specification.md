...
   - **GitHub Actions**
1. Require **no modifications to individual `.csproj` files ideally**:
   - Preferred installation is repo-level (`Directory.Build.props` / central package management), not per-project edits.
1. Output computed version strings to the **build log** every build.
2. Version **NuGet packages** produced by `dotnet pack`.
3. Version-stamp **assemblies** (version attributes).
4. Generate an injected **static class** compatible with GitVersion:
...
1. Provide **default GitFlow branch semantics**:
...
1. Produce a **unique version string for every commit**.
...
It may be worth exploring alternative approaches, as GitVersion's approach is occasionally problematic. We can ignore legacy .NET Framework builds and focus on building SDK-style projects targetting .NETStandard2.0 and .Net 8.0 and up. This may release us from backward-compatibility constraints imposed by .NET Framework. However, if no clearly superior approach can be found, we must adopt the same pattern as GitVersion but keep configuration and behaviour minimal.
...
**Primary package**: `Timtek.GitFlowVersioning`

Contents:
- `build/Timtek.GitFlowVersioning.props`
- `build/Timtek.GitFlowVersioning.targets`
- `buildTransitive/Timtek.GitFlowVersioning.props`
- `buildTransitive/Timtek.GitFlowVersioning.targets`
- MSBuild task assemblies (multi-targeted)
- any helper executables if needed (optional; see below)
  

**Optional secondary package**: `Timtek.GitFlowVersioning.Tool` (dotnet tool) 
...
- `versionTagRegex`: `^(?<prefix>[vV]?)(?<version>\d+\.\d+\.\d+)$`
...
Commits on master/main branches are assumed to be deployable releases, therefore should not have any prerelease tag. Every commit shall increment the Patch version unless overriden by a tag.

For other branches, the tool MUST produce a unique version for every commit. This is achieved by:

- using a commit-distance based numeric component (e.g., `PreReleaseNumber` or `BuildMetaData`), and

- always including a SHA-bearing `FullBuildMetaData` component. GitVersion’s `FullBuildMetaData` concept explicitly combines build metadata with branch and SHA. [2](https://gitversion.net/docs/reference/variables)
...
Compute a “version source” commit and base version:

1. Find the latest SemVer tag reachable from the current commit (preferably from main’s first-parent history). GitVersion’s GitFlow docs treat tags on main as stable releases. [3](https://gitversion.net/docs/learn/branching-strategies/gitflow/) 
2. Parse `Major.Minor.Patch` from the tag.
3. If no tag exists, use `fallbackBaseVersion` (default `0.1.0`), aligning with GitVersion’s GitFlow doc fallback behavior description for main when no merge/tag is found. [3](https://gitversion.net/docs/learn/branching-strategies/gitflow/) 

Compute:

- `VersionSourceSha`
- `CommitsSinceVersionSource` (a.k.a. distance / height)
...
- For non-tagged commits on main/master, the tool MUST still be unique per commit by incrementing the Patch version (note: build metadata is not part of the semantic version and cannot therefore produce uniqueness)
...
- `PreReleaseNumber` MUST be commit distance from `VersionSourceSha` (or branch-local height)

- `SemVer` MUST include `-beta.{n}`

- `FullSemVer` MUST include the prerelease and may include build metadata

- This aligns with GitVersion’s GitFlow doc which describes hotfix branches producing beta prereleases with an incrementing count. [3](https://gitversion.net/docs/learn/branching-strategies/gitflow/) 
...
- `PreReleaseNumber` MUST ensure uniqueness (commit-distance based)
...
   - `Timtek.GitFlowVersioning.AssemblyInfo.g.cs` (or set equivalent MSBuild properties)
...
- Generate **no namespace** (global namespace), so consuming code can reference:
